...
interface AddNewtour {
  ...
  setVisitTimings: ({
    ...
    goodStart: string;
    goodEnd: string;
    notRecomendedStart: string;
    notRecomendedEnd: string;
  }) => void;
  addTourInDB: ({
    ...
    heroBannerContentHeadAndPara: HeroBannerContent;
    heroBannerImageurls: HeroBannerImageUrl[];
...
const useAddNewTour = create(
  persist<AddNewtour>(
    (set) => ({
      ...
      reset: () => {
        set({
          heroBannerContent: { briefParagraph: "", heading: "" },
          heroBannerImageurls: [],
          mainBackImageUrl: "",
          placeName: "",
          pricing: { premium: 0, standard: 0 },
          thingsTodoArr: [],
          visitTimings: {
            best: { end: "", start: "" },
            good: { end: "", start: "" },
            notRecomended: { end: "", start: "" },
          },
        });
      },
      ...
      addTourInDB: async ({
        ...
        try {
          ...
          if (res.success) {
            ...
            set({
              isLoading: false,
              tourAddingError: true,
              tourAddingErrorMessage: res.message,
            });
          }
        } catch (error) {
          console.log(error);
          set({
            isLoading: false,
            tourAddingError: true,
            tourAddingErrorMessage: "Something went wrong",
          });
        }
      },
